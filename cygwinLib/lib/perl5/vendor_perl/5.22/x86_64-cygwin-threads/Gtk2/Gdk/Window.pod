...
=head2 pattern = $window-E<gt>B<get_background_pattern> 
...
=head2 boolean = $window-E<gt>B<get_composited> 
...
=head2 (x, y) = $window-E<gt>B<coords_from_parent> ($parent_x, $parent_y)

=over

=item * $parent_x (double) 

=item * $parent_y (double) 

=back

Since: gtk+ 2.22

=head2 (parent_x, parent_y) = $window-E<gt>B<coords_to_parent> ($x, $y)
...
=head2 surface = $window-E<gt>B<create_similar_surface> ($content, $width, $height)

=over

=item * $content (Cairo::Content) 
...
=head2 cursor or undef = $window-E<gt>B<get_cursor> 
...
=head2 window = $window-E<gt>B<get_effective_parent> 

Since: gtk+ 2.22

=head2 window = $window-E<gt>B<get_effective_toplevel> 

Since: gtk+ 2.22

=head2 window or undef = $offscreen_window-E<gt>B<get_embedder> 

Only for offscreen C<Gtk2::Gdk::Windows>.

Since: gtk+ 2.18

=head2 $offscreen_window-E<gt>B<set_embedder> ($embedder)

=over

=item * $embedder (Gtk2::Gdk::Window) 
...
=head2 boolean = $window-E<gt>B<ensure_native> 
...
=head2 $window-E<gt>B<geometry_changed> 
...
=item * $geometry (scalar) Gtk2::Gdk::Geometry or hashref
...
=item * $leader (Gtk2::Gdk::Window or undef) 

=back

=head2 boolean = $window-E<gt>B<has_native> 
...
=item * $shape (Gtk2::Gdk::Region or undef) 
...
=head2 boolean = $window-E<gt>B<is_destroyed> 

Since: gtk+ 2.18

=head2 boolean = $window-E<gt>B<is_input_only> 

Since: gtk+ 2.22

=head2 boolean = $window-E<gt>B<is_shaped> 
...
=head2 boolean = $window-E<gt>B<get_modal_hint> 
...
=head2 pixmap or undef = $offscreen_window-E<gt>B<get_pixmap> 
...
=head2 $window-E<gt>B<restack> ($sibling, $above)

=over

=item * $sibling (Gtk2::Gdk::Window or undef) 
...
=head2 (root_x, root_y) = $window-E<gt>B<get_root_coords> ($x, $y)
...
=item * $shape_region (Gtk2::Gdk::Region or undef) 
